...
(test range-listener-test-2 ()
	...
	(multiple-value-bind (li current-range changed) (funcall r '(1 a 3))	  
	  (is (equal li '((1 a 3) (1 a 3))))
	  (is (zerop current-range))
	  (is-false changed))
	
	(multiple-value-bind (li current-range changed) (funcall r '(1 3 r))	  
	  (is (equal li '((1 a 3) (1 a 3))))
	  (is (zerop current-range))
	  (is-false changed))

	(multiple-value-bind (li current-range changed) (funcall r '(2 r d))	  
	  (is (equal li '((1 a 3) (2 r d))))
	  (is (equal current-range 1))
	  (is-true changed))

	(multiple-value-bind (li current-range changed) (funcall r '(0 3 e))	  
	  (is (equal li '((2 r d) (0 3 e))))
	  (is (equal current-range -2))
	  (is-true changed))

	(multiple-value-bind (li current-range changed) (funcall r '(-1 -11 -111))	  
	  (is (equal li '((2 r d) (-1 -11 -111))))
	  (is (equal current-range -3))
	  (is-true changed))

	(multiple-value-bind (li current-range changed) (funcall r '(10 11 100 9))
	  (is (equal li '((-1 -11 -111) (10 11 100 9))))
	  (is (equal current-range 11))
	  (is-true changed))))
